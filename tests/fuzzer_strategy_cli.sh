...
grep -q 'GameFilterTCP' "$ROOT_DIR/tachyon/files/usr/lib/diagnostics/flowseal_import.uc" || fail "Flowseal importer must resolve GameFilterTCP"
grep -q 'GameFilterUDP' "$ROOT_DIR/tachyon/files/usr/lib/diagnostics/flowseal_import.uc" || fail "Flowseal importer must resolve GameFilterUDP"
grep -q 'replace(token, "\^!", "!")' "$ROOT_DIR/tachyon/files/usr/lib/diagnostics/flowseal_import.uc" || fail "Flowseal importer must remove Windows caret escaping"
...
grep -q 'function replace_all_literal' "$FUZZER" || fail "fuzzer must replace every Flowseal fake path occurrence"
